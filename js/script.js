...
function mostrarBox() {
    ...
    loadTrack(track_index);
    // Set the initial volume to 0 for fade-in effect
    curr_track.volume = 0;

    // Play the audio after user interaction (button click triggers the start function)
    curr_track.play().then(() => {
        console.log("Audio is playing");
        fadeInAudio(curr_track, 12000,0.0001);  // Fade-in over 5 seconds
    }).catch(error => {
...
function start() {
    ...
    curr_track.play().then(() => {
        console.log("Audio is playing");
        fadeInAudio(curr_track, 8000,0.05);  // Fade-in over 5 seconds
    }).catch(error => {
        console.log("Error playing audio:", error);
    });

    // Start the flower animations and content after audio plays
    setTimeout(() => {
        //const wrappers = document.querySelectorAll('.wrapper');
        ...
        flowers.forEach((flower, index) => {
            ...
            wrappers.forEach((wrapper, index) => {
                ...
                }, index * 2500); // Delay between each flower animation
            });
        });
        ...
        // Fade out black screen and show content
        setTimeout(() => {
            blackScreen.classList.add('fade-out');
    ...
    }, 3000); // Delay before animations start
}

// Function to gradually increase the volume (fade-in)
function fadeInAudio(audio, duration,steps) {
    let volume = 0;
    const step = steps; // Increment step (change to make the fade smoother/faster)
    const intervalTime = duration / (1 / step); // Calculate interval based on duration
    const fadeInInterval = setInterval(() => {
        if (volume < 1) {
            volume = Math.min(volume + step, 1); // Ensure volume doesn't exceed 1
            audio.volume = volume;
        } else {
